...
required_vars=(
  ...
  CLOUD_SQL_EDITION CLOUD_SQL_TIER CLOUD_SQL_DATABASE CLOUD_SQL_MAINTENANCE_USER
  CLOUD_SQL_RUNTIME_USER CLOUD_SQL_BACKUP_START_TIME CLOUD_SQL_DELETION_PROTECTION
  ...
  DEPLOY_SERVICE_ACCOUNT GITHUB_REPOSITORY GITHUB_ALLOWED_REFS
  GITHUB_ALLOWED_ENVIRONMENTS WIF_POOL_ID WIF_PROVIDER_ID SCHEDULER_JOB_NAME
  ...
  RUNTIME_SECRET_BINDINGS MIGRATE_SECRET_ENV_NAMES
)
...
exact mutation commands it will run and requires a typed yes before executing
them. No secret value is generated or printed.
EOF
...
if ! gcloud sql instances describe "$CLOUD_SQL_INSTANCE" --project="$GCP_PROJECT_ID" \
  ...
  sql_create=(
    ...
    --tier="$CLOUD_SQL_TIER" --availability-type=zonal --assign-ip
...
# exist, so they queue as their own section rather than joining it.
...
for database_user in "$CLOUD_SQL_MAINTENANCE_USER" "$CLOUD_SQL_RUNTIME_USER"; do
  existing_user=$(gcloud sql users list --instance="$CLOUD_SQL_INSTANCE" \
    --project="$GCP_PROJECT_ID" --filter="name=${database_user}" --format='value(name)' \
    --limit=1 --quiet)
  if [[ "$existing_user" != "$database_user" ]]; then
    plan gcloud sql users create "$database_user" --instance="$CLOUD_SQL_INSTANCE" \
      --type=BUILT_IN --project="$GCP_PROJECT_ID" --quiet
  fi
done
...
execute_section "Cloud SQL database and users"
...
if ! gcs_value storage buckets describe "gs://${GCS_PUBLIC_ASSETS_BUCKET}" \
  ...
  plan_gcs storage buckets update "gs://${GCS_PUBLIC_ASSETS_BUCKET}" \
    --project="$GCP_PROJECT_ID" --uniform-bucket-level-access --quiet
fi
...
service_accounts=(
  ...
  "$MIGRATE_SERVICE_ACCOUNT" "$DEPLOY_SERVICE_ACCOUNT"
)
...
storage_permissions='storage.buckets.create,storage.buckets.get,storage.buckets.update,storage.objects.create,storage.objects.delete,storage.objects.get,storage.objects.list,storage.objects.update'
...
declare -A migrate_secret_ids=(["$DATABASE_MAINTENANCE_URL_SECRET_ID"]=1)
while IFS= read -r migrate_env_name; do
  ...
  migrate_secret_ids["$matched_secret_id"]=1
done < <(csv_values "$MIGRATE_SECRET_ENV_NAMES")
for secret_id in "${!migrate_secret_ids[@]}"; do
...
plan gcloud projects add-iam-policy-binding "$GCP_PROJECT_ID" \
  --member="serviceAccount:${DEPLOY_SERVICE_ACCOUNT}" --role=roles/run.admin \
  --condition=None --quiet
plan gcloud artifacts repositories add-iam-policy-binding "$ARTIFACT_REGISTRY_REPOSITORY" \
  --project="$GCP_PROJECT_ID" --location="$GCP_REGION" \
  --member="serviceAccount:${DEPLOY_SERVICE_ACCOUNT}" \
  --role=roles/artifactregistry.writer --quiet
for service_account in "${service_accounts[@]:0:4}"; do
  plan gcloud iam service-accounts add-iam-policy-binding "$service_account" \
    --project="$GCP_PROJECT_ID" --member="serviceAccount:${DEPLOY_SERVICE_ACCOUNT}" \
    --role=roles/iam.serviceAccountUser --quiet
done
execute_section "Service accounts and IAM bindings"

echo "Checking GitHub Actions Workload Identity Federation"
if ! gcloud iam workload-identity-pools describe "$WIF_POOL_ID" --location=global \
  --project="$GCP_PROJECT_ID" --format='value(name)' --quiet >/dev/null 2>&1; then
  plan gcloud iam workload-identity-pools create "$WIF_POOL_ID" --location=global \
    --project="$GCP_PROJECT_ID" --display-name="Praxis GitHub Actions" --quiet
fi
ref_condition=""
while IFS= read -r allowed_ref; do
  [[ "$allowed_ref" =~ ^refs/(heads|tags)/[A-Za-z0-9._/-]+$ ]] \
    || die "invalid GITHUB_ALLOWED_REFS entry: $allowed_ref"
  [[ -z "$ref_condition" ]] || ref_condition+=" || "
  ref_condition+="assertion.ref=='${allowed_ref}'"
done < <(csv_values "$GITHUB_ALLOWED_REFS")
environment_condition=""
while IFS= read -r allowed_environment; do
  [[ "$allowed_environment" =~ ^[A-Za-z0-9._-]+$ ]] \
    || die "invalid GITHUB_ALLOWED_ENVIRONMENTS entry: $allowed_environment"
  [[ -z "$environment_condition" ]] || environment_condition+=" || "
  environment_condition+="assertion.environment=='${allowed_environment}'"
done < <(csv_values "$GITHUB_ALLOWED_ENVIRONMENTS")
attribute_condition="assertion.repository=='${GITHUB_REPOSITORY}' && (${ref_condition}) && (${environment_condition})"
...
if ! gcloud iam workload-identity-pools providers describe "$WIF_PROVIDER_ID" \
  ...
  --format='value(name)' --quiet >/dev/null 2>&1; then
  plan gcloud iam workload-identity-pools providers create-oidc "$WIF_PROVIDER_ID" \
    --workload-identity-pool="$WIF_POOL_ID" --location=global --project="$GCP_PROJECT_ID" \
    --display-name="Praxis GitHub Actions" --issuer-uri=https://token.actions.githubusercontent.com \
  ...
  provider_state=$(gcloud iam workload-identity-pools providers describe "$WIF_PROVIDER_ID" \
    --workload-identity-pool="$WIF_POOL_ID" --location=global --project="$GCP_PROJECT_ID" \
    --format='value(state)' --quiet)
  [[ "$provider_state" == "ACTIVE" ]] \
    || die "existing WIF provider must be ACTIVE before it can be reconciled"
  plan gcloud iam workload-identity-pools providers update-oidc "$WIF_PROVIDER_ID" \
    --workload-identity-pool="$WIF_POOL_ID" --location=global --project="$GCP_PROJECT_ID" \
    --display-name="Praxis GitHub Actions" --issuer-uri=https://token.actions.githubusercontent.com \
    --attribute-mapping="$attribute_mapping" --attribute-condition="$attribute_condition" --quiet
fi
wif_member="principalSet://iam.googleapis.com/projects/${GCP_PROJECT_NUMBER}/locations/global/workloadIdentityPools/${WIF_POOL_ID}/attribute.repository/${GITHUB_REPOSITORY}"
plan gcloud iam service-accounts add-iam-policy-binding "$DEPLOY_SERVICE_ACCOUNT" \
  --project="$GCP_PROJECT_ID" --member="$wif_member" \
  --role=roles/iam.workloadIdentityUser --quiet
execute_section "Workload Identity Federation"
...
trap 'rm -f "$policy_input" "$policy_output"' EXIT
...
gcs_run storage buckets describe "gs://${GCS_PUBLIC_ASSETS_BUCKET}" \
  --project="$GCP_PROJECT_ID" --format='yaml(name,location,iamConfiguration)'
gcs_run storage buckets get-iam-policy "gs://${GCS_PUBLIC_ASSETS_BUCKET}" \
  --project="$GCP_PROJECT_ID" --format='yaml(bindings)'
run gcloud iam workload-identity-pools providers describe "$WIF_PROVIDER_ID" \
  --workload-identity-pool="$WIF_POOL_ID" --location=global --project="$GCP_PROJECT_ID" \
...
echo "Bootstrap complete. Secret resources are intentionally versionless."
echo "Populate them with the commands in deploy/gcp/README.md, set both Cloud SQL user passwords, then deploy."
