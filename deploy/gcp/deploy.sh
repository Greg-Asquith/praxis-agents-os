...
OUTLOOK_MAIL_OAUTH_TENANT=${OUTLOOK_MAIL_OAUTH_TENANT:-${MICROSOFT_GRAPH_TENANT:-}}
OUTLOOK_CALENDAR_OAUTH_TENANT=${OUTLOOK_CALENDAR_OAUTH_TENANT:-${MICROSOFT_GRAPH_TENANT:-}}
SHAREPOINT_OAUTH_TENANT=${SHAREPOINT_OAUTH_TENANT:-${MICROSOFT_GRAPH_TENANT:-}}
ANTHROPIC_VERTEX_AI=${ANTHROPIC_VERTEX_AI:-false}
VERTEX_PARTNER_MODELS_ENABLED=${VERTEX_PARTNER_MODELS_ENABLED:-false}
GOOGLE_VERTEX_LOCATION=${GOOGLE_VERTEX_LOCATION:-auto}
ANTHROPIC_VERTEX_LOCATION=${ANTHROPIC_VERTEX_LOCATION:-global}
set +a
...
required_vars=(
  ...
  INTEGRATIONS_OAUTH_REDIRECT_URI MICROSOFT_GRAPH_TENANT
  MICROSOFT_GRAPH_REQUESTS_PER_SECOND
  OUTLOOK_MAIL_OAUTH_CLIENT_ID OUTLOOK_CALENDAR_OAUTH_CLIENT_ID
  SHAREPOINT_OAUTH_CLIENT_ID SHAREPOINT_DISCOVERY_MAX_SITES
  ENCRYPTION_KEYS_SECRET_NAME
  CREDENTIAL_MASTER_KEY_SECRET_NAME DEFAULT_MODEL_PROVIDER DEFAULT_MODEL
  GOOGLE_VERTEX_AI GOOGLE_VERTEX_LOCATION
  ANTHROPIC_VERTEX_AI ANTHROPIC_VERTEX_LOCATION VERTEX_PARTNER_MODELS_ENABLED VERTEX_PARTNER_MODEL_LOCATIONS_YAML
  CONVERSATION_NAMING_PROVIDER CONVERSATION_NAMING_MODEL
  AGENT_HISTORY_SUMMARY_MODEL_PROVIDER AGENT_HISTORY_SUMMARY_MODEL
  KB_ANNOTATION_PROVIDER KB_ANNOTATION_MODEL
  ...
  WORKER_DRAIN_MAX_SECONDS WORKER_TASK_TIMEOUT_SECONDS
...
for boolean_variable in GOOGLE_VERTEX_AI ANTHROPIC_VERTEX_AI VERTEX_PARTNER_MODELS_ENABLED; do
...
for pool_size_variable in DB_POOL_SIZE DB_MAINTENANCE_POOL_SIZE AI_USAGE_DB_POOL_SIZE \
  AGENT_RUN_MAX_CONCURRENT_TURNS; do
...
turn_pool_limit=$(( worker_runtime_pool_capacity - 4 ))
(( AGENT_RUN_MAX_CONCURRENT_TURNS <= turn_pool_limit )) \
  || die "AGENT_RUN_MAX_CONCURRENT_TURNS must not exceed the runtime database pool capacity minus four ($turn_pool_limit)"
...
TEMPLATE_VARIABLES='${GCP_PROJECT_ID} ${GCP_PROJECT_NUMBER} ${GCP_REGION} ${API_IMAGE} ${WEB_IMAGE} ${CLOUD_SQL_CONNECTION_NAME} ${API_SERVICE_ACCOUNT} ${WEB_SERVICE_ACCOUNT} ${WORKER_SERVICE_ACCOUNT} ${MIGRATE_SERVICE_ACCOUNT} ${DATABASE_MAINTENANCE_URL_SECRET_ID} ${DEPLOYMENT_ENVIRONMENT} ${GCS_PUBLIC_ASSETS_BUCKET} ${GCS_WORKSPACE_BUCKET_LOCATION} ${WORKSPACE_BUCKET_PREFIX} ${PUBLIC_ASSETS_BASE_URL} ${APP_BASE_URL} ${FRONTEND_URL} ${ALLOWED_CORS_ORIGINS} ${COOKIE_DOMAIN} ${ARTIFACT_ORIGIN} ${ARTIFACT_SHARING_ENABLED} ${ALLOW_SIGNUP} ${SUPER_ADMIN_EMAILS} ${ALLOW_WORKSPACE_CREATION} ${EMAIL_AUTH_ENABLED} ${GOOGLE_OAUTH_ENABLED} ${GOOGLE_OAUTH_CLIENT_ID} ${GOOGLE_OAUTH_REDIRECT_URI} ${GITHUB_OAUTH_ENABLED} ${GITHUB_OAUTH_CLIENT_ID} ${GITHUB_OAUTH_REDIRECT_URI} ${MICROSOFT_OAUTH_ENABLED} ${MICROSOFT_OAUTH_CLIENT_ID} ${MICROSOFT_OAUTH_REDIRECT_URI} ${EMAIL_PROVIDER} ${EMAIL_ENABLED} ${EMAIL_REPLY_TO} ${INTEGRATIONS_ENABLED_PROVIDERS} ${INTEGRATIONS_OAUTH_REDIRECT_URI} ${MICROSOFT_GRAPH_TENANT} ${MICROSOFT_GRAPH_REQUESTS_PER_SECOND} ${OUTLOOK_MAIL_OAUTH_CLIENT_ID} ${OUTLOOK_MAIL_OAUTH_TENANT} ${OUTLOOK_CALENDAR_OAUTH_CLIENT_ID} ${OUTLOOK_CALENDAR_OAUTH_TENANT} ${SHAREPOINT_OAUTH_CLIENT_ID} ${SHAREPOINT_OAUTH_TENANT} ${SHAREPOINT_DISCOVERY_MAX_SITES} ${ENCRYPTION_KEYS_SECRET_NAME} ${CREDENTIAL_MASTER_KEY_SECRET_NAME} ${DEFAULT_MODEL_PROVIDER} ${DEFAULT_MODEL} ${GOOGLE_VERTEX_AI} ${GOOGLE_VERTEX_LOCATION} ${CONVERSATION_NAMING_PROVIDER} ${CONVERSATION_NAMING_MODEL} ${AGENT_HISTORY_SUMMARY_MODEL_PROVIDER} ${AGENT_HISTORY_SUMMARY_MODEL} ${EMBEDDINGS_PROVIDER} ${EMBEDDINGS_MODEL} ${EMBEDDINGS_DIMENSIONS} ${METRICS_ENABLED} ${TRUSTED_PROXY_CIDRS} ${AUDIT_EVENTS_RETENTION_DAYS} ${SECURITY_EVENTS_RETENTION_DAYS} ${AGENT_RUN_APPROVAL_EXPIRY_DAYS} ${API_CPU} ${API_MEMORY} ${WEB_CPU} ${WEB_MEMORY} ${JOB_CPU} ${JOB_MEMORY} ${DB_POOL_SIZE} ${DB_POOL_MAX_OVERFLOW} ${DB_MAINTENANCE_POOL_SIZE} ${DB_MAINTENANCE_POOL_MAX_OVERFLOW} ${WORKER_DRAIN_MAX_SECONDS} ${WORKER_TASK_TIMEOUT_SECONDS} ${WORKER_MAX_CONCURRENT_RUNS} ${WORKER_MAX_RETRIES} ${RUNTIME_SECRET_ENV_YAML} ${RUNTIME_SECRET_ANNOTATION} ${MIGRATE_SECRET_ENV_YAML} ${MIGRATE_SECRET_ANNOTATION}'
TEMPLATE_VARIABLES='${GCP_PROJECT_ID} ${GCP_PROJECT_NUMBER} ${GCP_REGION} ${API_IMAGE} ${WEB_IMAGE} ${CLOUD_SQL_CONNECTION_NAME} ${API_SERVICE_ACCOUNT} ${WEB_SERVICE_ACCOUNT} ${WORKER_SERVICE_ACCOUNT} ${MIGRATE_SERVICE_ACCOUNT} ${DATABASE_MAINTENANCE_URL_SECRET_ID} ${DEPLOYMENT_ENVIRONMENT} ${GCS_PUBLIC_ASSETS_BUCKET} ${GCS_WORKSPACE_BUCKET_LOCATION} ${WORKSPACE_BUCKET_PREFIX} ${PUBLIC_ASSETS_BASE_URL} ${APP_BASE_URL} ${FRONTEND_URL} ${ALLOWED_CORS_ORIGINS} ${COOKIE_DOMAIN} ${ARTIFACT_ORIGIN} ${ARTIFACT_SHARING_ENABLED} ${ALLOW_SIGNUP} ${SUPER_ADMIN_EMAILS} ${ALLOW_WORKSPACE_CREATION} ${EMAIL_AUTH_ENABLED} ${GOOGLE_OAUTH_ENABLED} ${GOOGLE_OAUTH_CLIENT_ID} ${GOOGLE_OAUTH_REDIRECT_URI} ${GITHUB_OAUTH_ENABLED} ${GITHUB_OAUTH_CLIENT_ID} ${GITHUB_OAUTH_REDIRECT_URI} ${MICROSOFT_OAUTH_ENABLED} ${MICROSOFT_OAUTH_CLIENT_ID} ${MICROSOFT_OAUTH_REDIRECT_URI} ${EMAIL_PROVIDER} ${EMAIL_ENABLED} ${EMAIL_REPLY_TO} ${INTEGRATIONS_ENABLED_PROVIDERS} ${INTEGRATIONS_OAUTH_REDIRECT_URI} ${ENCRYPTION_KEYS_SECRET_NAME} ${CREDENTIAL_MASTER_KEY_SECRET_NAME} ${DEFAULT_MODEL_PROVIDER} ${DEFAULT_MODEL} ${GOOGLE_VERTEX_AI} ${GOOGLE_VERTEX_LOCATION} ${ANTHROPIC_VERTEX_AI} ${ANTHROPIC_VERTEX_LOCATION} ${VERTEX_PARTNER_MODELS_ENABLED} ${VERTEX_PARTNER_MODEL_LOCATIONS_YAML} ${CONVERSATION_NAMING_PROVIDER} ${CONVERSATION_NAMING_MODEL} ${AGENT_HISTORY_SUMMARY_MODEL_PROVIDER} ${AGENT_HISTORY_SUMMARY_MODEL} ${KB_ANNOTATION_PROVIDER} ${KB_ANNOTATION_MODEL} ${EMBEDDINGS_PROVIDER} ${EMBEDDINGS_MODEL} ${EMBEDDINGS_DIMENSIONS} ${METRICS_ENABLED} ${TRUSTED_PROXY_CIDRS} ${AUDIT_EVENTS_RETENTION_DAYS} ${SECURITY_EVENTS_RETENTION_DAYS} ${AGENT_RUN_APPROVAL_EXPIRY_DAYS} ${API_CPU} ${API_MEMORY} ${WEB_CPU} ${WEB_MEMORY} ${JOB_CPU} ${JOB_MEMORY} ${DB_POOL_SIZE} ${DB_POOL_MAX_OVERFLOW} ${DB_MAINTENANCE_POOL_SIZE} ${DB_MAINTENANCE_POOL_MAX_OVERFLOW} ${AI_USAGE_DB_POOL_SIZE} ${AGENT_RUN_MAX_CONCURRENT_TURNS} ${WORKER_DRAIN_MAX_SECONDS} ${WORKER_TASK_TIMEOUT_SECONDS} ${WORKER_MAX_CONCURRENT_RUNS} ${WORKER_MAX_RETRIES} ${RUNTIME_SECRET_ENV_YAML} ${RUNTIME_SECRET_ANNOTATION} ${MIGRATE_SECRET_ENV_YAML} ${MIGRATE_SECRET_ANNOTATION}'
