"""Add classification to the AI usage purpose vocabulary.

Revision ID: core_0042
Revises: core_0041
"""
...
_PURPOSES_AFTER = (*_PURPOSES_BEFORE[:4], "classification", *_PURPOSES_BEFORE[4:])
...
def upgrade() -> None:
    """Allow native classifier helper usage rows."""
    _replace_purpose_check(_PURPOSES_AFTER)


def downgrade() -> None:
    """Restore the previous closed purpose vocabulary."""
    op.execute("DELETE FROM ai_usage_events WHERE purpose = 'classification'")
