# apps/api/integrations/notion/tools/update_page_content.py

"""Update Notion page content through an approval-only audited mutation."""
...
from .mutations import NotionReplacementRecords
from .schemas import NotionUpdatePageContentOutput
from .utils import (
...
REPLACEMENT_COLUMNS = (
    ToolFieldColumn(key="old_text", label="Find", required=True),
    ToolFieldColumn(key="new_text", label="Replace with"),
    ToolFieldColumn(
        key="replace_all",
        label="Replace all",
        options=("no", "yes"),
...
async def notion_update_page_content(
    ctx: RunContext[RuntimeDeps],
    page: Annotated[
        NotionPageReference,
        Field(description="Scoped Notion page reference returned by search."),
    ],
    replacements: NotionReplacementRecords,
) -> dict[str, Any]:
    async def operation(entry: ResolvedContextEntry, references) -> Any:
        ...
        if not isinstance(selected_page, NotionPageReference):
            raise TypeError("Notion content updates require a page reference")
        client: NotionClient | None = None
        prepared: UpdatePageMarkdownPreparation | None = None
        ...
        async def prepare_pending_operation():
            ...
            return pending_update_content_detail(entry, prepared)
        ...
        return await run_audited_integration_operation(
            ctx,
            entry,
            tool_name="notion_update_page_content",
...
DEFINITION = RuntimeToolDefinition(
    ...
    function=notion_update_page_content,
    description="Replace exact text in one selected writable Notion page.",
    provider="notion",
    label="Update Notion Page Content",
    code_eligible=True,
    ...
    takes_ctx=True,
    timeout=90,
    output_model=NotionUpdatePageContentOutput,
    integration_binding=NOTION_WRITE_BINDING,
    availability_check=notion_available,
    presentation=ToolPresentation(
        icon="notion",
        running_label="Updating Notion Page Content",
        completed_label="Updated Notion Page Content",
        failed_label="Couldn't Update Notion Page Content",
        approval_title="Update Notion Page Content",
        approval_prompt="The agent wants to replace this text in Notion.",
