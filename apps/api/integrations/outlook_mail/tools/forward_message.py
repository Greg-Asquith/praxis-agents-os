# apps/api/integrations/outlook_mail/tools/forward_message.py

"""Forward Outlook email through the shared approval and audit runtime."""
...
from .mutations import ForwardInput, MailBody, Recipients
from .schemas import ForwardOutput
from .utils import (
...
async def outlook_mail_forward_message(
    ctx: RunContext[RuntimeDeps],
    message: Annotated[OutlookMessageReference, Field(description="Message to forward.")],
    to: Recipients,
    body_html: MailBody = "",
) -> dict:
    try:
        args = ForwardInput(message=message, to=to, body_html=body_html)
    ...
    async def operation(entry):
        ...
        async def prepare() -> PreparedMailWrite:
            ...
            pending = pending_write_detail(
                entry,
                action="forward",
                fields={"recipient_count": len(args.to)},
            ...
            async def mutate() -> None:
                ...
                await reject_draft_attachments(
                    client, message_id=state.message_id, operation="forward_message"
                )
        ...
        return await run_audited_integration_operation(
            ctx,
            entry,
            tool_name="outlook_mail_forward_message",
...
DEFINITION = RuntimeToolDefinition(
    ...
    function=outlook_mail_forward_message,
    description=(
        "Forwards an Outlook email to the specified recipients, with an optional message above "
        "the quoted conversation. Select exactly one Outlook mailbox. Forward drafts with "
        "attachments must be sent in Outlook."
    ),
    provider="outlook_mail",
    label="Forward Outlook email",
    code_eligible=True,
    ...
    takes_ctx=True,
    timeout=90,
    output_model=ForwardOutput,
    integration_binding=OUTLOOK_MAIL_WRITE_BINDING,
    availability_check=outlook_mail_available,
    approval_display_args=partial(mutation_display_args, ForwardInput),
    presentation=ToolPresentation(
        icon="outlook_mail",
        running_label="Forwarding Outlook email",
        completed_label="Forwarded Outlook email",
        failed_label="Could not forward Outlook email",
        approval_title="Forward Outlook email",
        approve_label="Approve & Forward",
        approval_prompt="Your message is sent above the quoted conversation.",
        arg_fields=(
            ToolFieldPresentation(
                key="message",
                label="Message",
