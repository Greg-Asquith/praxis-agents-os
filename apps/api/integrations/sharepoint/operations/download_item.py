...
async def download_item(
    ...
    if item_kind(item, operation="read_file") != "file":
        raise file_error(
            "Select a file to read. Folders and packages cannot be read.", "unsupported_type"
        )
    result = item_result(item, drive_id=drive_id, operation="read_file")
    require_file_citation(item.get("webUrl"))
    ...
    if result["size_bytes"] > max_bytes:
        raise file_error(
            "This file exceeds the download limit. Select a smaller file.", "too_large"
        )
    if not isinstance(url, str) or not url:
        raise invalid_response("read_file")
    try:
        data = await client.get_bytes(url, operation="read_file", max_bytes=max_bytes)
