# apps/api/services/auth/oauth/start_oauth_link.py

"""Create an OAuth authorization URL for linking a provider to the current user."""
...
from services.auth.oauth.utils import create_oauth_link_state, resolve_provider_redirect_uri
...
async def start_oauth_link(
    ...
    *,
    ...
    state, expires_at = create_oauth_link_state(
    ...
    await record_auth_security_event(
        ...
        request=request,
        user_email=user.email,
        details={"provider": provider_name, "intent": "link", "redirect_uri": redirect_uri},
    )
