...
async def record_integration_operation_audit_event(
    *,
    ...
    """Records provider evidence independently or in the local mutation transaction.
    ...
    try:
        ...
        async with session_factory() as audit_db:
            await set_session_tenant_context(
                audit_db, workspace_id=workspace_id, user_id=run.user_id
            )
            audit_db.add(event)
        ...
        if raise_on_error or db is not None:
            raise
        return None
