...
async def test_liveness_has_version_and_does_not_check_database(db_async_client, monkeypatch):
    ...
    response = await db_async_client.get("/healthz")

    assert response.status_code == 200
    assert response.json() == {"status": "ok", "version": settings.APP_VERSION}


async def test_readiness_reports_ready(db_async_client, monkeypatch):
    async def ready() -> None:
        return None

    monkeypatch.setattr("routes.health.readiness.check_database_connection", ready)

    response = await db_async_client.get("/readyz")

    assert response.status_code == 200
    assert response.json() == {"status": "ready"}


async def test_readiness_reports_dependency_failure(db_async_client, monkeypatch):
