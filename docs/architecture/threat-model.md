...
| **(d) Integration-fetched content**                  | Provider API payloads such as Gmail and Outlook bodies, Outlook attachment text and people, SharePoint file and folder metadata and AnyDoc-converted file content, Airtable records, Search Console queries, pages, sitemap paths, canonical URLs, and referring URLs are attacker-controlled and become model-visible through integration read tools.                    | Provider free text crosses dispatch as a typed provenance node containing a server-minted source kind and `ref` (for example a Gmail message id or Search Console site URL). Nodes are persisted and streamed; an always-loaded, request-only model wrapper renders them with the shared markers in a copied request context immediately before provider dispatch. Size remains bounded by provider product limits and dispatch truncation.                                                                                                                                                                                                                                                                                                                                                                                                | The hostile email-body, converted SharePoint DOCX, Search Console row, and Search Console referring-URL fixtures retain their content inside nodes; every model request encloses them in the byte-stable shared frame, neutralizes forged markers at render time, and retains server-minted provenance. Persistence and SSE fixtures contain nodes rather than rendered frames.                                                                                                                                                                                                                                                                                                                | The model does not follow instructions embedded in provider content and reports the attempt.                                                                                                          |
...
a cancellable process with a 30-second deadline and retains AnyDoc's fixed
parser limits. Timeout and cancellation kill the worker. Signed download URLs
...
direct and Code Mode execution. These tests establish mechanical containment;
live-model resistance to document instructions remains unverified.
