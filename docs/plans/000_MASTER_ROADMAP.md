...
cascade retention are in place for enabled datasets. Slices C–D remain
pending; live sandbox QA for Slice A was unavailable because no local
...
| 089  | Google BigQuery provider per D14: workspace-shared service-account connections (reuses the 038 connect flow and the generalized Google token helper), dataset discovery → context groups, the `integration_table_schemas` cache + `integrations.sync_table_schemas` job (manifest-declared trigger seam), and three read tools — cached `list_tables`/`get_table_schema` plus a dry-run-gated, SELECT-only, byte/row-capped `run_query` with results untrusted-framed under Gate G6. **Slices A–B DONE 2026-07-28:** package, service-account generalization, bounded client, dataset discovery, schema cache, bounded sync job, trigger wiring, and retention. Slices C–D remain pending. Read-only v1; semantic schema search, value profiling, and a verified-query library are recorded follow-ups.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
