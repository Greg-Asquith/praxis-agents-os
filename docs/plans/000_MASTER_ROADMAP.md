...
- Verify/refresh statuses of 010–020 (verified 2026-07-02: 010 is DONE;
  011–020 remain TODO; only the `skills` table and delegation pre-exist).
...
| 012 | Stream thinking live over SSE | P1 | Now — next hardening item / user-visible gap |
...
| Plan | Scope | Priority |
|------|-------|----------|
| 021 | Schedule REST routes: CRUD, pause/enable, run-now, run history, awaiting-approval visibility. Backend worker already exists; this is the missing product surface. | P1 |
| 022 | Schedules management UI: list, editor (prompt/cron/timezone), run history with statuses, approval-resume visibility. Active-context selection is added later by 040. | P1 |
...
| Plan | Scope |
|------|-------|
| 025 | `ToolDefinition` contract + decorator registry + import-time uniqueness/invariant checks; migrate the two catalog tools; write-time validation of `Agent.tool_names`/`tool_policies`; registry read API. (Donor A1.) |
...
`0 → 012 → 011 → 021 → 022 → 023 → 025 → 026 → 027 → 016 → 017 →
