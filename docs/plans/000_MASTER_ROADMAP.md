...
| 041b | Structured untrusted content + rich provider tool UI: keep frame markers model-only by persisting/streaming typed provenance nodes, then use the packaging-note §5 presenter seam for Gmail rows, Ads reports, and Airtable record tables; add a generic provider-contributed ephemeral preview route rendering sanitized email HTML in a script-less sandboxed iframe. Added 2026-07-22; all 041 provider prerequisites are now satisfied. **Slices A–B DONE 2026-07-22:** structured nodes now persist and stream while byte-stable frames are rendered only at model-request assembly; shared kits and Gmail search/read/send presenters now provide stable, inbox-like, bounded in-Praxis workflows. **Slice C DONE 2026-07-23:** the preview backend, automatic read-message HTML rendering, and centred search-row popover drill-in are complete. **Slice D DONE 2026-07-23:** shared table/KPI kits and rich Google Ads report, account, and governed campaign-status presenters are complete; Google Ads tool results are ordinary typed values with no model-side untrusted framing or legacy decoder, per operator decision. **Slice E DONE 2026-07-23:** an operator decision superseded provider-specific chart toggles with an always-auto-mounted core `build_chart` tool, a compact defaulted multi-axis contract, theme-aware colours with optional brand hex overrides, native Recharts v3 rendering, compact model acknowledgements, and full-figure PNG export. Slice F remains TODO. |
