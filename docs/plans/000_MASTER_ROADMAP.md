...
boundary with no untrusted-content warnings. Slice D remains pending; live
sandbox QA for Slice A was unavailable because no local
BigQuery credential is configured.
...
| 089  | Google BigQuery provider per D14: workspace-shared service-account connections (reuses the 038 connect flow and the generalized Google token helper), dataset discovery → context groups, the `integration_table_schemas` cache + `integrations.sync_table_schemas` job (manifest-declared trigger seam), and three read tools — cached `list_tables`/`get_table_schema` plus a dry-run-gated, SELECT-only, byte/row-capped `run_query` returning plain typed data from operator-controlled databases. **Slices A–C DONE 2026-07-28:** package, service-account generalization, bounded client, dataset discovery, schema cache and sync, three context-scoped tools, query safety bounds, and per-resource audit. Slice D remains pending. Read-only v1; semantic schema search, value profiling, and a verified-query library are recorded follow-ups.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
