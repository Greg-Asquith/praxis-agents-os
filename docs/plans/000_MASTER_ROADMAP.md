...
| 041b | Structured untrusted content + rich provider tool UI: keep frame markers model-only by persisting/streaming typed provenance nodes, then use the packaging-note §5 presenter seam for Gmail rows, Ads reports, and Airtable record tables; add a generic provider-contributed ephemeral preview route rendering sanitized email HTML in a script-less sandboxed iframe. Added 2026-07-22; all 041 provider prerequisites are now satisfied. **Slices A–B DONE 2026-07-22:** structured nodes now persist and stream while byte-stable frames are rendered only at model-request assembly; shared kits and Gmail search/read/send presenters now provide stable, inbox-like, bounded in-Praxis workflows. **Slices C–E DONE 2026-07-23:** safe HTML preview/drill-in, rich Google Ads presenters, and the engine-owned `build_chart` tool are complete. **Slice F DONE 2026-07-24:** Airtable record field tables and governed create/update presenters now close the provider UI set, with the provider checklist and follow-ups updated. Operator-added Slice G (built-in delegation/skill/file rows on the kit substrate) remains TODO, so the plan stays in progress. |
