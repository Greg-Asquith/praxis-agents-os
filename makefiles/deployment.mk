.PHONY: compose-up
compose-up: local-env ## Build and run Postgres, API, worker, and web through Docker Compose
	$(COMPOSE) up --build postgres api worker web

.PHONY: compose-up-detached
compose-up-detached: local-env ## Build and run the full Compose stack in the background
	$(COMPOSE) up -d --build postgres api worker web
...
compose-logs: ## Follow logs for the Compose stack
	$(COMPOSE) logs -f postgres api worker web
