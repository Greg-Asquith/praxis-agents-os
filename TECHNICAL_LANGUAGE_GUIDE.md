...
Use UK English in all prose, including user-facing copy, documentation,
comments, and docstrings. Preserve exact code identifiers, commands, URLs,
and official names when their spelling differs. Do not use em dashes.
...
- **Use present tense.** Reserve "will" for actions that genuinely happen
  ...
  the backup process runs." Never use "will" for ordinary behaviour ("The
  server will send an acknowledgement") or hypothetical "would".
...
- **Don't anthropomorphise software.** Systems don't "see", "think", "want",
...
- **Plain words over fancy ones**: "use" not "utilise" or "leverage",
...
- **Spell out abbreviations on first use** with the short form in
  parentheses, such as "recovery point objective (RPO)", except universally known
...
- **No excessive claims.** Avoid superlatives and absolutes ("best",
  ...
  products.
- **Be prescriptive.** Recommend the simplest way to do a task rather than
  cataloguing every option. Requirement words carry exact meaning:
  - **must**: required.
  - **we recommend**: suggested; use "should" only for widely recognised
    best practice.
  - **can**: optional.
...
- **Don't surface internal identifiers, stack traces, table names, or HTTP
  ...
  reauthorised", not "Received `401 Unauthorized` from the OAuth token
  endpoint."
...
- For a sequence of steps, use a numbered list, with one action per step, in the
...
- One idea per paragraph, key point first, five to six sentences at most.
  Left-align everything; never centre or justify.
...
- Email addresses: `dana@example.com`. Choose a name from the diverse,
  gender-neutral set (Alex, Amal, Ariel, Charlie, Dana, Kai, Kim, Lee, Noam, Quinn, Sasha,
...
- IPv4: the RFC 5737 ranges (192.0.2.0/24, 198.51.100.0/24, 203.0.113.0/24).
  IPv6: 2001:db8::/32.
- Company: "Example Organisation". Avoid `foo`/`bar`/`baz`. Use descriptive
  names.

### Dates, numbers, and units

- Dates are unambiguous: "19 January 2017", or ISO 8601 (`2017-04-15`) where
  a numeric form is needed. Never "02/12/2017". No seasons ("autumn 2026") because
...
- Refer to figures by number ("as shown in figure 1"), never by position
  ("the image above").
- Write link text and headings that make sense out of context; keep colour
...
- Do not use em dashes. Use commas, colons, parentheses, or separate
  sentences instead. Do not use en dashes in prose. Use hyphens for number
  ranges ("5-10 minutes").
- Avoid semicolons where a period works; avoid parentheses for anything
  important because some readers skip them.
- Straight quotation marks, not curly. Commas and periods inside quotation
  ...
  outside. Prefer code font instead of quotes.
...
- **Keep comments terse, with a single line where possible.** Match the density and
  idiom of the surrounding file; this codebase favours sparse, short comments
  over block prose.
- Comments describe runtime behaviour and durable design decisions, not
...
| greyed out, disabled | unavailable |
| dialog box | dialog |
